...
const MyListing = () => {
  ...
  // Update the useEffect that handles fetching listings
  useEffect(() => {
      ...
      try {
        ...
        switch (activeTab) {
          case "Active":
            ...
            break;
          case "Inactive":
            params.status = "IN_ACTIVE"; // Note: Using IN_ACTIVE instead of INACTIVE
            break;
          case "Draft":
            params.status = "DRAFT";
            break;
        ...
        await dispatch(getmylisting(params));
    ...
    // Only fetch if we're on one of these tabs
    if (["All Products", "Active", "Inactive", "Draft"].includes(activeTab)) {
      fetchListings();
    }
  }, [dispatch, activeTab, debouncedSearchQuery]);
  ...
  const updatedTabs = tabs.map((tab) => {
    switch (tab.status) {
      ...
      case "IN_ACTIVE": // Update this to match the API's expected value
